...
description: This workflow plugin allows a mass upload of files with automatically assigned to the correct Goobi processes either on the basis of the file names or on the basis of the analysed barcodes.
published: true
keywords:
    - Goobi workflow
    - Plugin
...
`instant-move`                  | If the value is set to `false` (default), the files are first copied to the destination folder and then deleted from the source. If, on the other hand, the value is set to `true`, the files are immediately moved from the source to the destination folder.
